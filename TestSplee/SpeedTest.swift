...
struct Logger {
init?<T>(@autoclosure value: () -> T) {
return nil
}
func put<T>(@autoclosure value: () -> T, @autoclosure function: () -> String = __FUNCTION__, @autoclosure file: () -> String = __FILE__, @autoclosure line: () -> Int = __LINE__) {
//print("<\(file):\(line) \(function)> \(value)")
}
}

let Log: Logger? = nil//Logger()
...
let bench = Benchmark()
for _ in 0..<cnt {
let _ = Logger(value: str)
}
bench.check(__FUNCTION__)
}
func loggerCheck() {
let str = "test"
...
func arrInsertAtLastCheck() {

let bench = Benchmark()
var arr = [Int]()
for i in 0..<arrAppendCnt {
arr.insert(i, atIndex: arr.count)
}
bench.check(__FUNCTION__)
}


func realmEmptyCountCheck(res: Results<User>) {
let bench = Benchmark()
for _ in 0..<realmCnt {
if res.count > 10000 {
}
...
func realmEmptyFirstCheck(res: Results<User>) {
...
func realmEmptyIsEmptyCheck(res: Results<User>) {
